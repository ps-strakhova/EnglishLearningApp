...
class WordsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        topicName?.let { textWelcome.text = it }
    ...
    private fun loadWords() {
        topicName?.let { topic ->
            lifecycleScope.launch {
                ...
                recyclerView.adapter = WordsAdapter(words) { word ->
                    // Навигация через NavController с передачей Bundle
                    val bundle = Bundle().apply {
                        putString("topicName", word.topic)
                        putInt("startIndex", words.indexOf(word))
                    }
                    findNavController().navigate(
                        R.id.action_wordsFragment_to_wordCardFragment,
                        bundle
                    )
                }
    ...
    private fun getCardDeclension(count: Int): String {
